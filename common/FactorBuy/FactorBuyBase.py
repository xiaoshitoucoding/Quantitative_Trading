...
from Position.AtrPosition import AtrPosition
from Slippage.SlippageBuyMean import SlippageBuyMean
from Trade.MLFeature import MlFeature
from CoreBase.Base import ParamBase
import six
...
class FactorBuyBase(six.with_metaclass(ABCMeta, ParamBase)):
    """
    ...
    def __init__(self, capital, kl_pd, combine_kl_pd, benchmark, **kwargs):
        ...
        # self.ump_manger = UmpManager(self)
        ...
        # self.rsi_xd = kwargs['rsi']
        self.buyindicator_list = []
    ...
    def _slippage_class_init(self, **kwargs):
        """滑点类构建"""
        # 滑点类，默认SlippageBuyMean
        self.slippage_class = kwargs.pop('slippage', SlippageBuyMean)
        # TODO 滑点类完善参数构建等需求
    ...
    def make_buy_order_ml_feature(self, day_ind):
        ...
        :return:
        """
        return MlFeature().make_feature_dict(self.kl_pd, self.combine_kl_pd, day_ind, buy_feature=True)
    ...
    # @Deprecated('kelly object now use dict to build, it will be remove next version!')
...
class FactorBuyTD(FactorBuyBase):
...
class FactorBuyXD(FactorBuyBase):

...
from FactorBuy.FactorBuyBase import FactorBuyTD, BuyCallMixin




class FactorBuyWD(FactorBuyTD, BuyCallMixin):
    """
        示例短线：日胜率均值回复策略
    ...
    def _make_buy_date(self, last_kl):
        ...
        """
        from Util import KLUtil

        self.buy_date_week = []
        # 计算周期内，周期的胜率
        last_dw = KLUtil.date_week_win(last_kl)
        ...
        if len(last_dw_vd) > 0:
            ...
            last_dwm = KLUtil.date_week_mean(last_kl)
            # 摘取满足胜率的last_dw_vd

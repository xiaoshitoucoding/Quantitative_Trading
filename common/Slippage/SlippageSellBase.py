...
class SlippageSellBase(six.with_metaclass(ABCMeta, object)):
    """非高频日内滑点卖出决策抽象基类"""

    def __init__(self, kl_pd_sell, factor_name):
        """
        :param kl_pd_sell: 交易当日的交易数据
        :param factor_name: FactorSellBase实例对象的factor_name
        """
...
"""是否开启跌停板滑点卖出价格特殊处理，默认关闭，外部修改如：py.slippage.ssb.g_enable_limit_down = True"""
g_enable_limit_down = False
"""
    初始设定跌停板卖出成交概率100%，这里也可以在计算完一次概率后，再使用成交量做二次概率计算，
    外部修改如：py.slippage.ssb.g_limit_down_deal_chance = 0.5，即修改为50%成功卖出概率
"""

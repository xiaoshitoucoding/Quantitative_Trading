...
from CoreBase.Env import EMarketDataSplitMode
from CoreBase import Env
from CoreBase import StockPb
from CoreBase.StockPb import code_to_symbol
# from ..SimilarBu import Similar
# from ..SimilarBu import ECoreCorrType
from CoreBase.Benchmark import Benchmark
from Util import ScalerUtil
# from ..UtilBu.Progress import do_clear_output
# from ..SimilarBu.Similar import from_local
from TLine.TLine import TLine
from Util.DTUtil import plt_show
...
def calc_similar(symbol, cmp_symbol, sum_rank=None, corr_jobs=(ECoreCorrType.E_CORE_TYPE_PEARS,
    ...
    """
    使用TLSimilar.calc_similar()函数计算返回的相关性数值是以目标股票所在市场为观察者，
    它不关心某一个股票具体相关性的数值的大小，calc_similar(a, b) 的工作流程如下：
    1.计算a与市场中所有股票的相关性
    2.将所有相关性进行rank排序
    3.查询股票b在rank序列中的位置，此位置值即为结果
    即TLSimilar.calc_similar返回值由0至1，这样的好处是通过计算usTSLA与usAAPL在所有股票中的相似度水平，会更全局客观的体现相关性
    ...
    if sum_rank is None:
        tmp_market = Env.g_market_target
        # 强制把市场设置为一样的
        Env.g_market_target = cs_symbol.market
        corr_df_dict = Similar.multi_corr_df(corr_jobs)
        # 恢复之前的市场
        Env.g_market_target = tmp_market
        """
            eg： corr_df_dict
            {'pears':
                            usBIDU    usFB  usGOOG  usNOAH  usSFUN  usTSLA  usVIPS  usWUBA
    ...
    if show:
        log_func = logging.info if Env.g_is_ipython else print
        log_func('{} similar rank score {} : {}'.format(symbol, cmp_symbol, rank_score))

        # 通过make_kl_df序列的接口获取两个金融时间序列
        mul_pd = SymbolPd.make_kl_df([symbol, cmp_symbol], n_folds=2)
        kl_pd = mul_pd[symbol]
        kl_pd_cmp = mul_pd[cmp_symbol]

        # 缩放到同一个数量级type_look='look_max'
        kl_pd, kl_pd_cmp = ScalerUtil.scaler_xy(kl_pd.close,
                                                   kl_pd_cmp.close,
                                                   type_look='look_max')
        ...
        # 通过distance构造技术线对象TLine，可视化几个技术线
        line = TLine(distance, line_name='{} distance {}'.format(symbol, cmp_symbol))
        line.show()
        # 可视化技术线拟合曲线及上下拟合通道曲线
...
def calc_similar_top(symbol, sum_rank=None, corr_jobs=(ECoreCorrType.E_CORE_TYPE_PEARS,
    ...
    if show:
        kl_pd = SymbolPd.make_kl_df(symbol, n_folds=1)
        # 获取top show_cnt个symbol金融时间序列
        mul_pd = SymbolPd.make_kl_df(rank_head.index, n_folds=1)

        for cmp_symbol in rank_head.index:
            # 迭代所有的收盘价格序列进行数据缩放
            kl_pd_close, kl_pd_cmp_close = ScalerUtil.scaler_xy(kl_pd.close,
...
def coint_similar(symbol, sum_rank=None, corr_jobs=(ECoreCorrType.E_CORE_TYPE_PEARS,
    ...
    # 使用symbol做标尺
    benchmark = Benchmark(symbol, n_folds=1)
    ...
    if show:
        ...
        close_panel_pd = ScalerUtil.scaler_matrix(close_panel_pd, mean_how=True)
        """
            ScalerUtil.scaler_matrix缩放后的数据矩阵如下所示
            eg： close_panel_pd
        ...
        close_panel_pd.plot(figsize=Env.g_plt_figsize)
        ...
        """
            ...
            ScalerUtil.scaler_std(close_panel_pd_cp - close_panel_pd):
        ...
        """
        regular_diff = ScalerUtil.scaler_std(close_panel_pd_cp - close_panel_pd)
        regular_diff.plot(figsize=Env.g_plt_figsize)
        ...
        close_regular = ScalerUtil.scaler_std(benchmark.kl_pd.close)

...
from TLine.TLExecute import shift_distance, bfgs_min_pos, support_resistance_pos, \
...
from CoreBase.Base import FreezeAttrMixin
from Util import RegUtil
from Util import StatsUtil
from Util.DTUtil import arr_to_numpy
...
class TLine(FreezeAttrMixin):
    ...
    def show(self):
        ...
        plt.subplots(figsize=Env.g_plt_figsize)
        # tl装载技术线本体
        plt.plot(self.tl)
    ...
    def is_up_trend(self, up_deg_threshold=5, show=True):
        ...
        :return: 是否上升趋势
        """
        valid = RegUtil.valid_poly(self.tl, poly=1, show=show)
        if valid:
            deg = RegUtil.calc_regress_deg(self.tl, show=show)
    ...
    def show_best_poly(self, zoom=False, show=show):
        ...
        具体详情RegUtil.search_best_poly
        :param zoom: 透传search_best_poly是否缩放x,y
        :param show: 是否进行可视化
        """
        best = RegUtil.search_best_poly(self.tl, zoom=zoom, show=show)
        if show:
    ...
    def show_least_valid_poly(self, zoom=False, show=True):
        ...
        具体详情RegUtil.least_valid_poly
        :param zoom: 透传least_valid_poly是否缩放x,y
        :param show: 是否进行可视化
        """
        least = RegUtil.least_valid_poly(self.tl, zoom=zoom, show=show)
        if show:
    ...
    def show_shift_distance(self, how=EShiftDistanceHow.shift_distance_close, step_x=1.0, show=True, show_log=True):
        ...
        路程终点点位值使用的计算方法并非得到最准确的'路程位移比'，实现详TLExecute.shift_distance
    ...
    def show_support_resistance_pos(self, best_poly=0, show=True):
        ...
        :param best_poly: 函数使用者可设置best_poly, 设置后就不使用RegUtil.search_best_poly寻找了,
                          详细阅TLExecute.support_resistance_pos
        :param show: 是否可视化
        :return: (技术线支撑位: support_pos, 技术线阻力位: resistance_pos)
        """

        # 首先通过demean将序列去均值
        demean_y = StatsUtil.demean(self.tl)
        # 阻力位序列要取反
    ...
    def show_support_resistance_select_k(self, best_poly=0, show=True):
        ...
        :param best_poly: 传递show_support_resistance_pos，
                          函数使用者可设置best_poly, 设置后就不使用RegUtil.search_best_poly寻找了
        :param show: 是否可视化显示
    ...
    def show_support_resistance_trend(self, best_poly=0, only_last=False, plot_org=False, show=True, show_step=False):
        ...
        套接：show_support_resistance_select_k－>support_resistance_predict
             ->TLExecute.plot_support_resistance_trend
        最终对技术线阻力位和支撑位进行绘制，注意show参数控制的是中间流程中的可视化，不包括
        最终阻力位和支撑的可视化
        :param best_poly: 传递show_support_resistance_pos，
                          函数使用者可设置best_poly, 设置后就不使用RegUtil.search_best_poly寻找了
        :param only_last: 透传TLExecute.plot_support_resistance_trend，控制只绘制时间序列中最后一个发现的阻力或支撑
        :param plot_org: 透传TLExecute.plot_support_resistance_trend，控制是否绘制线段还是直线，控制是否绘制线段还是直线，

...
from CoreBase.PdHelper import pd_rolling_std, pd_ewm_mean, pd_ewm_std, pd_resample
...
def calc_wave_std(kl_pd, xd=21, ewm=True, show=True):
    ...
    # 主要目的就是通过roll_std构造TLine对象line
    line = TLine(roll_std, 'wave std')
    if show:
        line.show()
...
def calc_wave_abs(kl_pd, xd=21, show=True):
    ...
    在默认xd＝21情况下，变成了月震荡幅度，使用StatsUtil.demean对
    月震荡幅度进行去均值操作后得到技术线demean_wave，TLine包裹
    技术线返回
    :param kl_pd: 金融时间序列，pd.DataFrame对象
    ...
    """
        eg: change_ratio_sum
            ...
            2016-07-07    10.79
    """
    # 使用StatsUtil.demean进行去均值操作
    demean_wave = StatsUtil.demean(change_ratio_sum)
    """
        eg: demean_wave
        2014-07-24    -1.6303
        2014-08-14     0.5697
...
def calc_wave_weight_abs(kl_pd, xd=21, span=3, show=True):
    ...
    继续对squared_wave进行时间加权得到技术线形成ewm_wave，TLine包裹技术线返回
    :param kl_pd: 金融时间序列，pd.DataFrame对象
    :param xd: 对波动进行重采样的周期，单位天，int
    ...
    line = TLine(ewm_wave, 'squared ewm wave')
    if show:
        # 计算pd_resample how='mean'只是为了_show_wave里面显示价格曲线
        xd_mean_close = pd_resample(kl_pd.close, xd_resample, how='mean')
        # 这里不使用TLine.show，因为需要绘制另一个对比line，价格均线xd_mean_close
...
def _show_wave(wave, above, wave_mean, below, xd_mean_close):
    ...
    不使用TLine.show，因为需要绘制另一个对比line，价格均线xd_mean_close，
    使用双坐标轴的方式进行可视化
    """
    with plt_show():

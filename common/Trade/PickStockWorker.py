...
from Trade.PickBase import PickStockWorkBase
# from CoreBase.Market import all_symbol
from PickStock.PickStockBase import PickStockBase



class PickStockWorker(PickStockWorkBase):
    """选股类"""
    ...
    def init_stock_pickers(self, stock_pickers):
        ...
        if stock_pickers is not None:
            for picker_class in stock_pickers:
                ...
                if not isinstance(picker, PickStockBase):
                    # 因子对象类型检测
                    raise TypeError('factor must base PickStockBase')

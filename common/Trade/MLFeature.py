...
# from ..CoreBu.Fixes import xrange, range, six
# from ..MarketBu import MarketDrawing
# from ..TLineBu import TLAtr
# from ..TLineBu import TLJump
# from ..TLineBu import TLWave
# from ..UtilBu import RegUtil
# from ..UtilBu import StrUtil


# 内置特征，趋势角度
g_deg_keys = [21, 42, 60, Env.g_market_trade_year]
# 内置特征，价格rank
g_price_rank_keys = [60, 90, 120, Env.g_market_trade_year]
# 内置特征，波动周期定义
g_wave_xd = 42
# 内置特征，波动取样个数
g_wave_key_cnt = 3
...
class FeatureDeg(FeatureBase, BuyFeatureMixin, SellFeatureMixin):
    ...
    def calc_feature(self, kl_pd, combine_kl_pd, day_ind, buy_feature):
        ...
        for dk in self.deg_keys:
            ...
            ang = RegUtil.calc_regress_deg(deg_close, show=False)
            # 标准化拟合角度值
...
class FeatureWave(FeatureBase, BuyFeatureMixin, SellFeatureMixin):
    ...
    def calc_feature(self, kl_pd, combine_kl_pd, day_ind, buy_feature):
        ...
        wave_wide = Env.g_market_trade_year
        ...
        for xd_ind in xrange(1, self.wave_key_cnt + 1):
            # wave_df固定为一年交易时间序列，xd是calc_wave_st内部计算rolling std的window值，详TLWave.calc_wave_std
            wave = TLWave.calc_wave_std(wave_df, xd=xd_ind * self.wave_xd, show=False)
...
class FeatureAtr(FeatureBase, BuyFeatureMixin):
    ...
    def calc_feature(self, kl_pd, combine_kl_pd, day_ind, buy_feature):
        ...
        atr_wide = Env.g_market_trade_year
        ...
        # 返回的atr特征键值对字典
        atr_dict = {}
        # 计算atr特征，详见TLAtr.calc_atr_std
        atr_std = TLAtr.calc_atr_std(atr_df, xd=self.atr_xd, show=False)
...
class FeatureJump(FeatureBase, BuyFeatureMixin, SellFeatureMixin):
    ...
    def calc_feature(self, kl_pd, combine_kl_pd, day_ind, buy_feature):
        ...
        jump_wide = Env.g_market_trade_year
        ...
        # 通过特征周期时间序列jump_df做为参数，计算跳空，返回jumps对象为pd.DataFrame对象，详见TLJump.calc_jump
        jumps = TLJump.calc_jump(jump_df, show=False)
        """
...
class FeatureSnapshot(FeatureBase, BuyFeatureMixin, SellFeatureMixin):
    ...
    def calc_feature(self, kl_pd, combine_kl_pd, day_ind, buy_feature):
        ...
        # 生成64位随机字符串
        rn = StrUtil.create_random_with_alpha(64)
        ...
        MarketDrawing.g_only_draw_price = True
        # 通过特征周期时间序列snap_window_pd做为参数，绘制交易k线图快照，保存在本地，详MarketDrawing.plot_candle_form_klpd
        MarketDrawing.plot_candle_form_klpd(snap_window_pd, save=True, name=snap_fn)
        # 快照特征键值对字典添加快照特征key和对应的快照特征值
        snap_dict = {'{}{}'.format(self.feature_prefix(buy_feature=buy_feature),
                                   self.snap_key): MarketDrawing.save_dir_name() + snap_fn + '.png'}
        return snap_dict
...
class MlFeature(object):
    ...
    def make_feature_dict(self, kl_pd, combine_kl_pd, day_ind, buy_feature):
        ...
        if not Env.g_enable_ml_feature:
    ...
    def unzip_ml_feature(self, orders_pd):
        ...
        if Env.g_enable_ml_feature:
            ...
            # from ..UtilBu.DTUtil import except_debug
            # @except_debug

...
from CoreBase.Env import EMarketDataSplitMode, EMarketDataFetchMode
...
def gen_dict_pick_time_kl_pd(target_symbols, capital, benchmark, show_progress=True):
    ...
    def _batch_gen_dict_pick_time_kl_pd():   
        for epoch, target_symbol in enumerate(target_symbols):
            ...
            kl_pd = StockPb.make_kl_df(target_symbol, benchmark=benchmark, n_folds=benchmark.n_folds)
...
class KLManager(object):
    ...
    def _fetch_pick_stock_kl_pd(self, xd, target_symbol):
        ...
        end = DateUtil.timestamp_to_str(self.benchmark.kl_pd.index[-1])

        if xd == Env.g_market_trade_year:
            ...
            n_folds = float(xd / Env.g_market_trade_year)
            # 为了计算start，xd的单位是交易日，换算为自然日
            delay_day = 365 * n_folds
            start = DateUtil.begin_date(delay_day, date_str=end, fix=False)
        ...
        if hasattr(self, pre_bc_key):
            ...
            pre_benchmark = Benchmark(n_folds=n_folds, start=start, end=end)
            # 类变量设置选股时段benchmark
            setattr(self, pre_bc_key, pre_benchmark)
        # 以选股时段benchmark做为参数，获取选股时段对应symbol的金融时间序列
        return StockPb.make_kl_df(target_symbol,benchmark=pre_benchmark, n_folds=pre_benchmark.n_folds, start=start, end=end)

    def _fetch_pick_time_kl_pd(self, target_symbol):
        """获取择时时段金融时间序列"""
        return StockPb.make_kl_df(target_symbol, benchmark=self.benchmark, n_folds=self.benchmark.n_folds)
    ...
    def batch_get_pick_time_kl_pd(self, choice_symbols, n_process= Env.g_cpu_cnt, show_progress=True):
        """
        ...
        if n_process <= 0:
            ...
            n_process = Env.g_cpu_cnt

        # TODO 需要区分hdf5和csv不同存贮情况，csv存贮模式下可以并行读写
        # 只有E_DATA_FETCH_FORCE_LOCAL才进行多任务模式，否则回滚到单进程模式n_process = 1
        if n_process > 1 and Env.g_data_fetch_mode != EMarketDataFetchMode.E_DATA_FETCH_FORCE_LOCAL:
            # 1. hdf5多进程还容易写坏数据
    ...
    def get_pick_stock_kl_pd(self, target_symbol, xd=Env.g_market_trade_year,
                             min_xd=int(Env.g_market_trade_year / 2)):
        """
...
def split_k_market(k_split, market_symbols=None):
    """

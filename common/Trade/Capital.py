...
from Trade.Commission import Commission
# from ..TradeBu.Order import Order
# from ..TradeBu.Commission import Commission
# from ..CoreBu.Base import PickleStateMixin
...
class Capital():
    ...
    def __init__(self, init_cash, benchmark, user_commission_dict=None):
        ...
        :param benchmark: 资金回测时间标尺，做为资金类表格时间范围确定使用，Benchmark实例对象
        :param user_commission_dict: dict对象，可自定义交易手续费计算方法，详情查看Commission
        """
        ...
        # 构造交易手续费对象Commission，如果user自定义手续费计算方法，通过user_commission_dict传入
        self.commission = Commission(user_commission_dict)
    ...
    def apply_init_kl(self, action_pd):
        """
    ...
    def apply_action(self, a_action):
        """
        ...
        if is_buy:
            # 如果是买单，sell_price = price2 ,详情阅读TradeExecute中transform_action
            order.buy_price = a_action['Price']
            order.sell_price = a_action['Price2']
        else:
            # 如果是卖单，buy_price = price2 ,详情阅读TradeExecute中transform_action
    ...
    def buy_stock(self, a_order):
        ...
        以及更新对应symbol上的持仓量
        :param a_order: 在apply_action中由action转换的Order对象
        :return: 是否成交deal bool
        """
        ...
        # 买单时间转换成pd时间日期对象
        time_ind = pd.to_datetime(DateUtil.fmt_date(a_order.buy_date))
    ...
    def sell_stock(self, a_order):
        ...
        # 卖单时间转换成pd时间日期对象
        time_ind = pd.to_datetime(DateUtil.fmt_date(a_order.sell_date))
        # # pd时间日期对象置换出对应的index number
        num_index = self.capital_pd.index.tolist().index(time_ind)
        # 根据a_order.expect_direction确定是要更新call的持仓量还是put的持仓量
        buy_type_keep = '_call_keep' if a_order.expect_direction == 1.0 else '_put_keep'

...
class OrderPdProxy(object):
    ...
    def proxy_work(self, orders_pd):
        """
        传人需要比较的orders_pd，构造OrderPdProxy对象，返回使用者，
        对op_result进行统一分析
        :param orders_pd: 回测结果生成的交易订单构成的pd.DataFrame对象
        :return:
        """
        ...
        # 运算集结果重置
        self.op_result = None
        # 实例化比较的OrderPdProxy对象
        other = OrderPdProxy(orders_pd)
        # try:
        #     yield self, other
        # finally:
        #     if isinstance(self.op_result, pd.DataFrame):
        #         # # 如果有并集, 交集, 差集运算结果存储，
        #         # from ..MetricsBu.MetricsBase import MetricsBase
        #         # metrics = MetricsBase(self.op_result, None, None, None)
...
def trade_summary(orders, kl_pd, draw=False, show_info=True):
    """
    主要将Order对象序列转换为pd.DataFrame对象orders_pd，以及将
    ...
    # Order对象序列转换为pd.DataFrame对象orders_pd
    orders_pd = TradeExecute.make_orders_pd(orders, kl_pd)
    # 交易单子时间序列转换交易行为顺序序列
    action_pd = TradeExecute.transform_action(orders_pd)

    summary = ''
    if draw:
        # 绘制每笔交易的细节交易图
        TradeDrawer.plot_his_trade(orders, kl_pd)

    if show_info:
        # simple的意思是没有计算交易费用
        simple_profit = 'simple profit: {} \n'.format(TradeExecute.calc_simple_profit(orders, kl_pd))

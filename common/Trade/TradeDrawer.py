...
def plot_his_trade(orders, kl_pd):
    """
    可视化绘制Order对象，绘制交易买入时间，卖出时间，价格，生效因子等
    ...
    for index, order in enumerate(orders):
        ...
        # 格式化买入信息标签
        buy_date_fmt = DateUtil.str_to_datetime(str(order.buy_date), '%Y%m%d')
        ...
        if order.sell_price is not None:
            ...
            sell_date_fmt = DateUtil.str_to_datetime(str(order.sell_date), '%Y%m%d')
            ...
            sell_date_fmt = DateUtil.str_to_datetime(str(all_pd[-1:]['date'][0]), '%Y%m%d')
        ...
        if not draw_multi_ax:
            # ipython环境绘制在多个子画布上，普通python环境绘制一个show一个
            plt.show()
            png_name = Env.g_picture_dir + 'plot_' + order.buy_symbol + '.png'
...
def plot_capital_info(capital_pd, init_cash=-1):
    """
    资金信息可视化
    :param capital_pd: Capital对象或者Capital对象的capital_pd
    :param init_cash: 初始化cash，如果capital_pd为Capital对象，即从capital_pd获取
    """

    if isinstance(capital_pd, Capital):
        # 如果是Capital对象进行转换
    ...
    plt.show()
    plt.savefig('./PlotPicture/stocks_blance.png')
    ...
    try:
        ...
        plt.show()
        plt.savefig('./PlotPicture/plot_capital_info.png')

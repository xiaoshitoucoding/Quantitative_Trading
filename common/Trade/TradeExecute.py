...
def make_orders_pd(orders, kl_pd):
    """
    Order对象序列转换为pd.DataFrame对象，order_pd中每一行代表一个Order信息
    :param orders: Order对象序列
    :param kl_pd: 金融时间序列，pd.DataFrame对象
    """
    ret_orders_pd = None
    for index, order in enumerate(orders):
        # 迭代order，将每一个Order对象转换为一个pd.DataFrame对象
    ...
    dates_fmt = list(map(lambda date: DateUtil.fmt_date(date), ret_orders_pd['buy_date'].tolist()))
    ...
    # # 如果单子开启了特征收集，将收集的特征添加到对应的交易中，详阅读MlFeature
    # MlFeature().unzip_ml_feature(ret_orders_pd)
...
def apply_action_to_capital(capital, action_pd, kl_pd_manager, show_progress=True):
    """
    多个金融时间序列对应的多个交易行为action_pd，在考虑资金类Capital对象的情况下，对Capital对象进行

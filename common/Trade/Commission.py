...
# from ..MarketBu.SymbolFutures import FuturesCn
# from six import partial
# from ..CoreBu import Env
# from ..CoreBu.Env import EMarketTargetType
# from .Order import OrderMarket
# from ..MarketBu import Market


# from six import partial

from CoreBase import Env
from CoreBase.Env import EMarketTargetType
from Trade.Order import OrderMarket
from CoreBase.Market import *
...
def calc_commission_tc(trade_cnt, price):
    ...
    如需要更精确计算，请使用自定义计算费率，即在Commission初始化中自定义计算手续费的方法
...
def calc_commission_futures_cn(trade_cnt, price, symbol_name):
    ...
    commission_unit = 10
    # 查询商品期货的对应df
    q_df = FuturesCn().query_symbol(symbol_name)
    if q_df is not None:
        # 每手单位数量
...
class Commission(object):
    ...
    def _commission_enter(self, a_order):
        ...
        :param a_order: 交易单对象Order实例
        :return:
        """

        # 如果使用env中统一设置，即不需要通过OrderMarket对单子查询市场，提高运行效率，详Market
        # market = Env.g_market_target if Market.g_use_env_market_set \
        ...
        if market == EMarketTargetType.E_MARKET_TARGET_US:
            ...
            calc_commission_func = calc_commission_futures_cn
    ...
    def buy_commission_func(self, a_order):
        ...
        if len(commission_list) == 1:
            ...
            # 将买单对象Order实例中的数据转换成交易记录需要的np.array对象
    ...
    def sell_commission_func(self, a_order):
        ...
        if len(commission_list) == 1:
            commission = commission_list[0]
            # 将卖单对象Order实例中的数据转换成交易记录需要的np.array对象

...
from CoreBase.StockPb import *
from FactorBuy.FactorBuyBase import FactorBuyBase
from FactorSell.FactorSellBase import FactorSellBase
from Trade.PickBase import PickTimeWorkBase
...
"""
    ...
    py.alpha.pick_time_worker.g_natural_long_task = False
"""
g_natural_long_task = True


# noinspection PyAttributeOutsideInit
class PickTimeWorker(PickTimeWorkBase):
    """择时类"""

    def __init__(self, cap, kl_pd, benchmark, buy_factors, sell_factors):
        """
        :param cap: 资金类Capital实例化对象
        ...
        self.combine_kl_pd = StockPb.combine_pre_kl_pd(self.kl_pd, n_folds=1)
        # 如特别在乎效率性能，打开下面注释的方式，只在g_enable_ml_feature模式下开启, 注释上一行
        # self.combine_kl_pd = SymbolPd.combine_pre_kl_pd(self.kl_pd,
        #                                                    n_folds=1) if Env.g_enable_ml_feature else None
        # 传递给因子系列，因子内部可有选择性使用
        ...
        # 择时最终买入卖出行为列表，列表中每一个对象都为Order对象
        self.orders = list()
    ...
    #         self.task_pg = MulPidProgress(len(self.kl_pd), 'pick {} times'.format(self.kl_pd.name))
    ...
    def init_sell_factors(self, sell_factors):
        ...
        for factor_class in sell_factors:
            ...
            if not isinstance(factor, FactorSellBase):
                # 因子对象类型检测
                raise TypeError('factor must base FactorSellBase')
            # 添加到卖出因子序列
    ...
    def init_buy_factors(self, buy_factors):
        ...
        for factor_class in buy_factors:
            ...
            if not isinstance(factor, FactorBuyBase):
                # 因子对象类型检测
                raise TypeError('factor must base FactorBuyBase')
            # 添加到买入因子序列

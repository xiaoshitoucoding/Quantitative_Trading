...
# from ..CoreBu.Env import EMarketDataFetchMode
# from ..CoreBu.Fixes import six
# from ..UtilBu import DateUtil
# from ..UtilBu import StatsUtil, ScalerUtil
# from ..UtilBu.DTUtil import warnings_filter
# from ..TradeBu.KLManager import KLManager
# from ..TradeBu.Capital import Capital
# from ..TradeBu import TradeExecute



from CoreBase import Env
from CoreBase.Env import EMarketDataFetchMode
import six
from Util import DateUtil
from Util import ScalerUtil, StatsUtil
from Trade.KLManager import KLManager
from Trade.Capital import Capital
from Trade import TradeExecute
from Util.DTUtil import warnings_filter
...
class MetricsBase(object):
    ...
    def __init__(self, orders_pd, action_pd, capital, benchmark, enable_stocks_full_rate_factor=False):
        ...
        # self.log_func = logging.info if Env.g_is_ipython else print
        self.log_func = print
    ...
    def fit_metrics(self):
        ...
        # with Progress(100, 0, label='metrics progress...') as pg:
        # pg.show(5)
    ...
    def _metrics_sell_stats(self):
        ...
        self.orders_pd['keep_days'] = self.orders_pd.apply(lambda x:
                                                           DateUtil.diff(x['buy_date'],
                                                                            DateUtil.current_date_int()
    ...
    def _metrics_action_stats(self):
        ...
        """
            ...
            67     20150717
        """
        dt_fmt = cp_date.apply(lambda order: DateUtil.str_to_datetime(str(order), '%Y%m%d'))
        ...
        if self.act_buy.empty:
            ...
            # 计算cost各种统计度量值
            self.cost_stats = StatsUtil.stats_namedtuple(self.act_buy['cost'])
    ...
    def plot_order_returns_cmp(self, only_info=True):
        ...
        plt.show()
        try:
            plt.savefig('./PlotPicture/plot_order_returns_cmp.png')
        except:
            print("savefig plot_order_returns_cmp error!!")
    ...
    def plot_returns_cmp(self, only_show_returns=False, only_info=False):
        ...
        csv_list = []
        csv_list.append(self.win_rate * 100) #胜率
        csv_list.append(self.win_loss_profit_rate) #盈亏比
        csv_list.append(self.algorithm_period_returns * 100) #策略收益
        csv_list.append(self.algorithm_annualized_returns * 100) #策略年化收益
        csv_list.append(self.benchmark_period_returns * 100)  #基准收益
        csv_list.append(self.algorithm_annualized_returns * 100) #基准年化收益
        csv_list.append(self.gains_mean * 100) #平均获利期望
        csv_list.append(self.losses_mean * 100) #平均亏损期望
        csv_list.append(self.order_has_ret.shape[0]) #买入后卖出的交易数量
        csv_list.append(self.order_keep.shape[0]) #买入后尚未卖出的交易数量
        csv_list.append(self.buy_deal_rate * 100) #策略买入成交比例
        csv_list.append(self.cash_utilization * 100) #策略资金利用率比例
        csv_list.append(self.num_trading_days) #策略共执行交易日
        ...
        # plt.show()
        # try:
        #     plt.savefig('./PlotPicture/benchmark_returns.png')
        # except:
        #     print("savefig benchmark_returns error!!")
        ...
        plt.show()
        try:
            plt.savefig('./PlotPicture/algorithm_cum_returns.png')
        except:
            print("savefig algorithm_cum_returns error!!")
        sns.distplot(self.capital.capital_pd['capital_blance'], kde_kws={"lw": 3, "label": "capital blance kde"})
        plt.show()
        try:
            plt.savefig('./PlotPicture/capital_blance.png')

        except:
            print("savefig capital_blance error!!")
        return csv_list
    ...
    def plot_effect_mean_day(self):
        ...
        plt.show()
        try:
            plt.savefig('./PlotPicture/plot_effect_mean_day.png')
        except:
            print("savefig plot_effect_mean_day error!!")
    ...
    def plot_action_buy_cost(self):
        ...
        plt.show()
        try:
            plt.savefig('./PlotPicture/plot_action_buy_cost.png')
        except:
            print("savefig plot_action_buy_cost error!!")
    ...
    def plot_sell_factors(self):
        ...
        if self.dumm_sell_t_sum.shape[0] > 1:
            ...
            plt.show()
            try:
                plt.savefig('./PlotPicture/plot_sell_factors.png')
            except:
                print("savefig plot_sell_factors error!!")
    ...
    def plot_buy_factors(self):
        ...
        if self.dumm_buy_t_sum.shape[0] > 1:
            ...
            plt.show()
            try:
                plt.savefig('./PlotPicture/plot_buy_factors.png')
            except:
                print("savefig plot_buy_factors error!!")
    ...
    def plot_keep_days(self):
        ...
        plt.show()
        try:
            plt.savefig('./PlotPicture/plot_keep_days.png')
        except:
            print("savefig plot_keep_days error!!")
    ...
    def plot_max_draw_down(self):
        ...
        plt.show()
        try:
            plt.savefig('./PlotPicture/plot_max_draw_down.png')
        except:
            print("savefig plot_max_draw_down error!!")

        self.log_func('最大回撤: {:5f}'.format(down_rate))
        self.log_func('最大回测启始时间:{}, 结束时间{}, 共回测{:3f}'.format(
            DateUtil.timestamp_to_str(list(six.iterkeys(max_draw_down))[0][0]),
            DateUtil.timestamp_to_str(list(six.iterkeys(max_draw_down))[0][1]),
            list(six.itervalues(max_draw_down))[0]))

    @valid_check
    def transform_to_full_rate_factor(self, read_cash=-1, kl_pd_manager=None, n_process_kl=Env.g_cpu_cnt,
                                      show=True):
        if Env.g_data_fetch_mode != EMarketDataFetchMode.E_DATA_FETCH_FORCE_LOCAL:
        ...
        if not hasattr(self, 'full_rate_metrics'):
            ...
            # 重新以很大的资金初始化Capital
            capital = Capital(read_cash, self.benchmark,
                                 user_commission_dict=self.capital.commission.commission_dict)
            if kl_pd_manager is None:
                kl_pd_manager = KLManager(self.benchmark, capital)
            ...
            TradeExecute.apply_action_to_capital(capital, action_pd, kl_pd_manager)
            # 最终创建一个子MetricsBase对象在内部，action_pd, capital使用新计算出来的，满仓乘数参数设置为True
            # noinspection PyAttributeOutsideInit
            self.full_rate_metrics = MetricsBase(self.orders_pd, action_pd, capital, self.benchmark,
...
class MetricsDemo(MetricsBase):
    """
        扩展自定义度量类示例

        eg:
            metrics = MetricsDemo(*_result_tuple)
            metrics.fit_metrics()
    ...
    def plot_commission(self):
        ...
        x, y = ScalerUtil.scaler_xy(self.commission_cum, self.algorithm_cum_returns, type_look='look_max',
                                       mean_how=True)
        ...
        plt.show()
        try:
            plt.savefig('./PlotPicture/plot_commission.png')
        except:
            print("savefig plot_commission error!!")

...
from Slippage.SlippageSellMean import SlippageSellMean
from Slippage.SlippageSellBase import SlippageSellBase
...
class FactorSellBase():
    ...
    def __init__(self, capital, kl_pd, combine_kl_pd, benchmark, **kwargs):
        ...
        # # 滑点类，默认SlippageSellMean
        self.slippage_class = kwargs.pop('slippage', SlippageSellMean)
    ...
    def make_sell_order_ml_feature(self, day_ind):
        """
         根据卖出交易发生的时间索引构通过MlFeature构建卖出时刻的各个交易特征
         :param day_ind: 交易发生的时间索引，对应self.kl_pd.key
         :return:
         """
        return MlFeature().make_feature_dict(self.kl_pd, self.combine_kl_pd, day_ind, buy_feature=False)
...
class FactorSellXD(FactorSellBase):

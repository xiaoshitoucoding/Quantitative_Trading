...
class ResultTuple(namedtuple('ResultTuple',
                                ('orders_pd',
                                 'action_pd',
                                 'capital',
                                 'benchmark'))):
...
def dump_custom_index_csv(custom_name, custom_desc):
    """
    ...
    # 数据保存在cache目录中
    index_csv_path = os.path.join(Env.g_project_data_dir, 'cache', 'custom_index.csv')
    FileUtil.ensure_dir(index_csv_path)

    index_csv_df = FileUtil.load_df_csv(index_csv_path)
    ...
    # 最终dump为csv文件
    FileUtil.dump_df_csv(index_csv_path, index_csv_df)
...
def dump_custom_ump_index_csv(custom_name, ump_unique, is_main_ump, custom_desc):
    ...
    # 数据保存在ump缓存目录
    index_csv_path = os.path.join(Env.g_project_data_dir, 'ump', 'custom_ump_index.csv')
    FileUtil.ensure_dir(index_csv_path)
...
def _load_custom_index(*paths):
    """执行读取csv，通过数据目录 + *paths参数构成读取路径"""
    index_csv_path = os.path.join(Env.g_project_data_dir, *paths)
    index_csv_df = FileUtil.load_df_csv(index_csv_path)
    return index_csv_df


def load_custom__index():
    """读取回测结果索引描述csv，通过数据目录 + cache + custom__index.csv构成读取路径"""
    return _load_custom_index('cache', 'custom__index.csv')


def load_custom_ump_index():
    """读取裁判ump训练索引描述csv，通过数据目录 + ump + custom_ump_index.csv构成读取路径"""
...
def del_custom__index(custom_name):
    """删除回测结果索引描述csv中某一特定行，custom_name为行名称，即执行drop"""
    return _del_custom_index(custom_name, 'cache', 'custom_index.csv')
...
def cache_result_path(n_folds, store_type, custom_name):
    """由外部参数返回所有单子存贮路径"""
    fn_root = Env.g_project_cache_dir
    fn_head = '' if n_folds is None else 'n{}_'.format(n_folds)

    # 根据EStore来决定fn_head
    if store_type == EStore.E_STORE_TEST:
        fn_head += 'test'
    elif store_type == EStore.E_STORE_TEST_UMP:
        fn_head += 'test_ump'
    elif store_type == EStore.E_STORE_TEST_UMP_WITH_EDGE:
        fn_head += 'test_ump_with_edge'
    elif store_type == EStore.E_STORE_TRAIN:
        fn_head += 'train'
    elif store_type == EStore.E_STORE_CUSTOM_NAME:
        fn_head += custom_name
    elif store_type != EStore.E_STORE_NORMAL:
    ...
    FileUtil.ensure_dir(orders_path)
...
def store_result_tuple(_result_tuple, n_folds=None, store_type=EStore.E_STORE_NORMAL,
                           custom_name=None):
    """
    ...
    """
    orders_path, orders_key, action_path, action_key, capital_path, benchmark_path = cache_result_path(
        n_folds, store_type, custom_name)
    # _result_tuple.orders_pd使用dump_hdf5存储
    FileUtil.dump_hdf5(orders_path, _result_tuple.orders_pd, orders_key)
    # _result_tuple.action_pd使用dump_hdf5存储
    FileUtil.dump_hdf5(action_path, _result_tuple.action_pd, action_key)
    # _result_tuple.capital使用dump_pickle存储Capital对象
    FileUtil.dump_pickle(_result_tuple.capital, capital_path)
    # _result_tuple.benchmark使用dump_pickle存储Benchmark对象
    FileUtil.dump_pickle(_result_tuple.benchmark, benchmark_path)
...
def load_result_tuple(n_folds=None, store_type=EStore.E_STORE_NORMAL, custom_name=None):
    ...
    orders_pd = FileUtil.load_hdf5(orders_path, orders_key)
    # load_hdf5读取pd.DataFrame对象action_pd
    action_pd = FileUtil.load_hdf5(action_path, action_key)
    # load_pickle读取Capital对象capital，Capital混入了类PickleStateMixin
    capital = FileUtil.load_pickle(capital_path)
    # load_pickle读取Benchmark对象benchmark，Benchmark混入了类PickleStateMixin
    benchmark = FileUtil.load_pickle(benchmark_path)
    # 构建返回ResultTuple对象
    return ResultTuple(orders_pd, action_pd, capital, benchmark)


def delete_result_tuple(n_folds=None, store_type=EStore.E_STORE_NORMAL, custom_name=None, del_index=False):
    """
    ...
    orders_path, _, action_path, _, capital_path, benchmark_path = _cache_result_path(
        n_folds, store_type, custom_name)

    # 获取各个单子路径后依次删除
    FileUtil.del_file(orders_path)
    FileUtil.del_file(action_path)
    FileUtil.del_file(capital_path)
    FileUtil.del_file(benchmark_path)

    if del_index:
        # 删除回测所对应的描述文件索引行
        del_custom_index(custom_name)


def store_result_out_put(result_tuple, show_log=True):
    """
    ...
    fn = os.path.join(Env.g_project_data_dir, base_dir, date_dir, 'orders.csv')
    FileUtil.ensure_dir(fn)

    fn = os.path.join(Env.g_project_data_dir, base_dir, date_dir, 'actions.csv')
    FileUtil.dump_df_csv(fn, _result_tuple.action_pd)
    if show_log:
        print('save {} suc!'.format(fn))

    fn = os.path.join(Env.g_project_data_dir, base_dir, date_dir, 'capital.csv')
    FileUtil.dump_df_csv(fn, _result_tuple.capital.capital_pd)
    if show_log:
        print('save {} suc!'.format(fn))

    fn = os.path.join(Env.g_project_data_dir, base_dir, date_dir, 'commission.csv')
    FileUtil.dump_df_csv(fn, _result_tuple.capital.commission.commission_df)
    if show_log:
        print('save {} suc!'.format(fn))

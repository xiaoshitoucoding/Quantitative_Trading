...
from CoreBase.Env import EMarketTargetType, IndexSymbol, EMarketDataSplitMode
...
class Benchmark():
    ...
    def __init__(self, benchmark=None, start=None, end=None, n_folds=2, rs=True, benchmark_kl_pd=None):
        ...
        if benchmark is None:
            if Env.g_market_target == EMarketTargetType.E_MARKET_TARGET_US:
                # 美股
            ...
            elif Env.g_market_target == EMarketTargetType.E_MARKET_TARGET_HK:
                # 港股
                benchmark = IndexSymbol.HSI
            elif Env.g_market_target == EMarketTargetType.E_MARKET_TARGET_CN:
                # a股
            ...
            elif Env.g_market_target == EMarketTargetType.E_MARKET_TARGET_FUTURES_CN:
                # 国内期货
                benchmark = IndexSymbol.BM_FUTURES_CN
            elif Env.g_market_target == EMarketTargetType.E_MARKET_TARGET_TC:
                # 币类市场
                benchmark = IndexSymbol.TC_INX
            elif Env.g_market_target == EMarketTargetType.E_MARKET_TARGET_OPTIONS_US:
                # 美股期权暂时也以IXIC做为标尺，最好是外部参数中的benchmark设置
                benchmark = IndexSymbol.IXIC
            elif Env.g_market_target == EMarketTargetType.E_MARKET_TARGET_FUTURES_GLOBAL:
        ...
        self.kl_pd = StockPb.make_kl_df(benchmark.code_id, n_folds=n_folds,
                                    start=start, end=end)

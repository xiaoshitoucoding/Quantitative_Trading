...
def getn_date(num,formats='%Y%m%d'):
    return (datetime.datetime.today()+datetime.timedelta(-num)).strftime(formats)

def getn_afterdate(num,formats='%Y%m%d'):
    return (datetime.datetime.today()+datetime.timedelta(num)).strftime(formats)
...
def diffdays_between(start_date):
...
def GetToday(formats="%Y-%m-%d"):
    today = datetime.datetime.today().strftime(formats)
    return today

def GetTomorrow(formats="%Y-%m-%d"):
    tomorrow = (datetime.datetime.today()+datetime.timedelta(1)).strftime(formats)
    return tomorrow

def GetAfterNDayBydate(start_date, num,formats="%Y-%m-%d"):
    start_date = fix_date(start_date)
    start_date = str_to_datetime(start_date)
    return (start_date+datetime.timedelta(num)).strftime(formats)
...
def _calc_start_end_date(df, force_local, n_folds, start, end):
    ...
    today = timestamp_to_str(df.index[-1]) if force_local else current_str_date()
    if end is None:
        # 没有end也没start，end＝today，否则使用n_folds计算end
        end = today if start is None else begin_date(-365 * n_folds, date_str=start, fix=False)
    # int类型的end, today转换
    ...
    if start is None:
        if force_local:
            ...
            if end_ss is None or end_ss.empty:
                ...
                # +1补上
                ind = end_ss.key.values[0] - (g_market_trade_year * n_folds) + 1
            if ind < 0:
                ind = 0
            # 强制本地，计算从第几个ind开始取df数据
            start = timestamp_to_str(df.index[ind])
        else:
            # 非强制本地，使用n_folds年数，向前推计算start
            start = begin_date(365 * n_folds, date_str=end, fix=False)
    start_int = date_str_to_int(start)
    ...
    if df is not None and df.shape[0] > 0:
        ...
        try:
            df_end = timestamp_to_str(df.index[-1])
            df_end_int = date_str_to_int(df_end)

            df_start = timestamp_to_str(df.index[0])
            df_start_int = date_str_to_int(df_start)
        except Exception as e:
            print("error!!", e)
    if end_int > df_end_int:
        end_int = df_end_int
...
def workdays_diff(start_date, end_date, check_order=True):
    """
    对两个输入日期计算间隔的天数，如果check_order=False, str日期对象效率最高
    :param start_date: str对象或者int对象，如果check_order=True int对象效率最高
    :param end_date: str对象或者int对象，如果check_order=True int对象效率最高

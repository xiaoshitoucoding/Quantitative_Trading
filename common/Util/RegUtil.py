...
from Util.StatsUtil import euclidean_distance_xy, manhattan_distances_xy, cosine_distances_xy
...
def regress_xy(x, y, mode=True, zoom=False, show=False):
    ...
    """
    if zoom:
        # 将y值 zoom到与x一个级别，不可用ScalerUtil.scaler_xy, 因为不管x > y还y > x都拿 x.max() / y.max()
        # TODO ScalerUtil中添加使用固定轴进行缩放的功能
        zoom_factor =abs(x.max()) / abs(y.max())
        y = zoom_factor * y

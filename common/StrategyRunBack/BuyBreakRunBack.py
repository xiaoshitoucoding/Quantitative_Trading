    ...
    for item_vol_rank in range(0, 11):
        ...
        for item_stop_loss in range(1, 10):
            for item_stop_win in range(1, 10):
                try:
                    FactorBuyBreakRunBack(csv_datalist=csv_datalist, vol_rank=item_vol_rank, stop_loss_n=item_stop_loss, stop_win_n=item_stop_win)
                except:
...
def FactorBuyBreakRunBack(csv_datalist, xd_1 = 50, xd_2 = 21, vol_rank = 0, stop_loss_n = 5.0, stop_win_n = 4.0):
    csv_columnslist = [xd_1, xd_2, vol_rank, stop_loss_n, stop_win_n]
    # 设置初始资金数
    read_cash = 1000000
    # 设置选股因子，None为不使用选股因子
    stock_pickers = None
    ...
    buy_factors = [{'xd': xd_1, 'rsi': 600, 'vol_rank': vol_rank, 'vol_daily': xd_1, 'class': FactorBuyBreak},
                {'xd': xd_2, 'rsi': 600, 'vol_rank': vol_rank, 'vol_daily': xd_2, 'class': FactorBuyBreak}
                ]
    ...
    metrics.fit_metrics()
    csv_columnslist += metrics.plot_returns_cmp()

from FactorBuy.FactorBuyTrend import UpDownTrend, UpDownGolden, DownUpTrend, DownTrendAndVolume
...
def BuyTrendRunBack():
    csv_datalist = []
    FactorBuyTrendRunBack(csv_datalist=csv_datalist, xd_1 = 20, vol_rank_min = -1, vol_rank_max=-0.5, stop_loss_n=2.0, stop_win_n=4.0)
    print(csv_datalist)
    # csv_datalist = []
...
def FactorBuyTrendRunBack(csv_datalist, xd_1 = 20, vol_rank_min = -1, vol_rank_max=0, stop_loss_n = 2.0, stop_win_n = 3.0):
    csv_columnslist = [xd_1, vol_rank_min, vol_rank_max, stop_loss_n, stop_win_n]
    # 设置初始资金数
    read_cash = 1000000
    # 设置选股因子，None为不使用选股因子
    stock_pickers = None
    ...
    # buy_factors = [{'xd': xd_1,  'vol_rank_min': vol_rank_min, 'vol_rank_max': vol_rank_max, 'vol_daily': xd_1,'class': UpDownTrend},
    #             ]

    # buy_factors = [{'xd': xd_1,  'vol_rank_min': vol_rank_min, 'vol_rank_max': vol_rank_max, 'vol_daily': xd_1,'class': DownTrendAndVolume},
    #             ]

    buy_factors = [{'xd': xd_1,  'vol_rank_min': vol_rank_min, 'vol_rank_max': vol_rank_max, 'decre_days': 3,'class': DownTrendAndVolume},
                    ]

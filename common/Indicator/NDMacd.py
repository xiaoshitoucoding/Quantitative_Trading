...
from Util.DTUtil import catch_error
from CoreBase.PdHelper import pd_ewm_mean
...
def plot_macd(prices, kl_index, with_points=None, with_points_ext=None,
    ...
    if with_price:
        plt.plot(kl_index, ScalerUtil.scaler_std(prices), label='close price')
...
def computeMACD(kl_pb):
    dif, dea, hist = _calc_macd_from_ta(kl_pb.close)
    df3 = pd.DataFrame({'dif':dif[33:],'dea':dea[33:],'hist':hist[33:]},
                       index=kl_pb['date'][33:],columns=['dif','dea','hist'])
    df3.plot(title='MACD')
    plt.show()
    plt.savefig('./PlotPicture/macd.png')
    #寻找MACD金叉和死叉
    datenumber = int(df3.shape[0])
    for i in range(datenumber-1):
        if ((df3.iloc[i,0]<=df3.iloc[i,1]) & (df3.iloc[i+1,0]>=df3.iloc[i+1,1])):
            print("MACD金叉的日期："+ str(df3.index[i+1]))
        if ((df3.iloc[i,0]>=df3.iloc[i,1]) & (df3.iloc[i+1,0]<=df3.iloc[i+1,1])):
            print("MACD死叉的日期："+ str(df3.index[i+1]))
    return(dif,dea,hist)
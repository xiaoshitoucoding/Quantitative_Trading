...
def before_market_open(context):
    ...
    if g.elapsed_days % ADJUST_INTERVAL == 0:  # 是否是调仓日
        ...
        for code, tail_value in g.tail_values.items():
            ...
            # 计算ATR指标
            df = get_price(code, end_date=tail_date, count=ATR_WINDOW_SIZE+1, frequency="1d", skip_paused=True, fields=["high", "low", "close"])
        ...
        # 计算每只个股承担的风险百分比
        sum = functools.reduce(lambda x, y: x + y["price"]/y["atr"], g.new_long_pos.values(), 0)
        # sum += functools.reduce(lambda x, y: x + y["price"]/y["atr"], g.new_short_pos.values(), 0)
        g.each_risk = 1 / sum

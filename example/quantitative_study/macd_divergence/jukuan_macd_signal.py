...
class DivergenceDetect:
    ...
    def get_divergences(self, df):
        ...
        if len(cdf) <= 1:  # 只找到一个极值点
            log.debug(u'【%s, %s】只有一个极值点, dif_limit_tm=%s' % (row.name, self.cross_type, row[DIF_LIMIT_TM]))
            return divergences

        dif, close, macd = self.get_limit_by_cross(df, row)
        if dif is None or close is None or macd is None:
            log.debug(u'【%s, %s】未找到穿越前的极值, dif_limit_tm=%s' % (row.name, self.cross_type, row[DIF_LIMIT_TM]))
            return divergences
        ...
        for i in range(len(cdf) - 2, -1, -1):
            ...
            if pre_dif is None or pre_close is None or pre_macd is None:
                log.debug(u'【%s, %s】未找到前一个背离点的极值, dif_limit_tm=%s' % (row.name, self.cross_type, row[DIF_LIMIT_TM]))
                continue
    ...
    def is_tangle_by_dea_and_dif(self, macd, pre_macd, macd_ser):
        ...
        if max([abs(macd[MACD]), abs(pre_macd[MACD])]) <= macd_max:
            log.debug(
                u'【%s, %s】与最大值相比，发生纠缠, %s, %s, %s' % (macd.name, pre_macd.name, macd[MACD], pre_macd[MACD], macd_max))
            return False
        return True
...
class Indicator:
    ...
    def get_limit_before_cross(self, df, current_bar_tm, pre_cross_type, limit_detect):
        ...
        if limit_df.empty:
            return None, None, None
        log.debug(u'[%s,%s]穿越点' % (current_bar_tm, pre_cross_tm))
...
class MacdCache:
    ...
    def __init_cache(self):
        ...
        for code in self.stocks:
            try:
                self.__init_single_cache(code, self.init_tm)
                log.debug(u'【macd缓存初始化end】code={}, init_tm={}'.format(code, self.init_tm))
            except:
                log.debug(u'【macd缓存初始化】异常：code={}, exception={}'.format(code, traceback.format_exc()))

    def __init_single_cache(self, code, init_tm):
        """
        初始化单支股票的缓存
        :param code:
        :param init_tm:
        :return:
        """
        df = self.dbkline.get_bars(code, count=self.bar_cache_num + EXTRA_LOAD_BAR_NUM, unit=self.period,
                                   fields=[CLOSE],
                                   end_tm=init_tm)
        if df.empty:
            log.debug(u'【macd缓存初始化】警告：code={},  bars empty'.format(code))
            return
    ...
    def update_last_bar_single_stock(self, df, code, idx, indicator):
        ...
        log.debug(u'【%s, %s】MACD更新完成, row=%s, divergences=%s' % (code, tm, row.to_dict(),
                                                                 Divergence.to_json_list(divergences)))
    ...
    def update_single_cache(self, code, last_tm, last_close):
        ...
        :return:
        """
        log.debug(u'【%s, %s】MACD查询数据close=%s，缓存更新' % (code, last_tm, last_close))
        ...
        if factors.empty:
            log.debug(u'【%s, %s】复权因子为空', code, last_tm)
            return
        ...
        if len(factors) > 1 and factors.iloc[0]['factor'] != factors.iloc[1]['factor']:
            self.__init_single_cache(code, last_tm)
            log.debug(u'【%s, %s】复权因子发生变化,重新初始化缓存', code, last_tm)
            return
        df = self.bars[code]
        if df.empty:
            return
    ...
    def update_divergences(self, df, code):
        ...
        """
        # 检测背离
        if code not in self.divergences.keys():
            self.divergences[code] = []
